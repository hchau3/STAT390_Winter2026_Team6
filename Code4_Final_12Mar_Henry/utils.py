...
class NumpyEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        if isinstance(obj, np.ndarray):
            return obj.tolist() # Converts numpy arrays to standard Python lists
        if isinstance(obj, (np.float32, np.float64, np.float16)):
            return float(obj)   # Converts numpy floats to standard Python floats
        if isinstance(obj, (np.int32, np.int64, np.int16, np.int8)):
            return int(obj)     # Converts numpy ints to standard Python ints
